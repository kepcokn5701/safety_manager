...
from backend.services.excel_parser import parse_excel, parse_worker_excel, extract_workers_from_site_data, parse_work_period
...
class SiteImportItem(BaseModel):
    ...
    work_period: str = ""    # 사전신고 '작업예정일시' 원문 (예: '2026-07-24 08:00 ~ 2026-07-26 18:00')
...
async def import_sites(
    ...
    for item in data.sites:
        # 작업예정일시 → 작업 시작/종료일 파싱
        work_start, work_end = parse_work_period(item.work_period)

        # 동일 이름의 기존 현장이 있는지 확인
        existing_site = await site_repo.get_by_name(item.name)
        if existing_site:
            # 기존 현장 → 작업예정일 갱신 (재업로드 시 최신 기간 반영)
            if work_start:
                existing_site.work_date_start = work_start
                existing_site.work_date_end = work_end
                await db.commit()
        ...
        try:
            site = await site_repo.create(
                ...
                is_outdoor=True,
                work_date_start=work_start,
                work_date_end=work_end,
            )
