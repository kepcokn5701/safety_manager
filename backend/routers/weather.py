...
from backend.services.repository import WorkSiteRepository, WeatherLogRepository, AlertLogRepository, site_work_status
...
class _CachedWeather:
    """실시간 기상청 조회가 실패했을 때, DB에 저장된 직전 정상 날씨(WeatherLog)를
    실시간 조회 결과처럼 다루기 위한 임시 래퍼.
    WeatherProvider가 돌려주는 WeatherResult와 동일한 속성 이름을 갖는다."""

    def __init__(self, log):
        self.temperature = log.temperature
        self.humidity = log.humidity
        self.wind_speed = log.wind_speed
        self.apparent_temperature = log.apparent_temperature
        self.wbgt_estimated = log.wbgt_estimated
        self.kma_base_time = log.kma_base_time or ""
...
async def get_all_weather_status(
    ...
    async def fetch_grid_weather(grid_key, grid_info):
        try:
            ...
            # 예외 메시지가 비어있는 경우(예: ConnectTimeout)에도 원인을 판별할 수 있도록
            # 예외 종류(클래스 이름)를 메시지 앞에 함께 담는다.
            return grid_key, None, f"{type(e).__name__}: {e}"
    ...
    for site in sites:
        try:
            ...
            # ── 캐시 폴백 ──
            # 실시간 기상청 조회가 실패했더라도(기상청 서버 일시 장애 등),
            # DB에 저장된 직전 정상 날씨가 있으면 그 값으로 표시한다.
            # (기상청이 잠깐 끊겼다고 전체 현장이 '조회 실패'로 빨개지는 것을 막음)
            weather_is_cached = False
            cached_label = ""
            if not weather:
                cached_log = await log_repo.get_latest_by_site(site.id)
                if cached_log and cached_log.temperature is not None:
                    weather = _CachedWeather(cached_log)
                    weather_is_cached = True
                    # recorded_at 은 UTC 기준으로 저장되므로 +9시간 해서 한국시각(KST)으로 표시
                    rec = cached_log.recorded_at or datetime.utcnow()
                    rec_kst = rec + timedelta(hours=9)
                    cached_label = f"{rec_kst.hour}시 {rec_kst.minute:02d}분 기준"

            # 실시간도 실패하고 캐시도 없을 때만 '조회 실패'로 처리
            if not weather:
                grid_err = grid_errors.get(grid_key, "")
                el = grid_err.lower()
                if "APPLICATION_ERROR" in grid_err or "NO_DATA" in grid_err:
                    reason = f"기상청에 해당 격자({nx},{ny}) 데이터가 없습니다 — 좌표({lat:.4f}, {lon:.4f})가 바다·도서 지역일 수 있습니다."
                elif "connecttimeout" in el or "connecterror" in el or "connectionerror" in el or "connection refused" in el:
                    reason = "기상청 서버에 연결할 수 없습니다 — 기상청 API 서버의 일시적 장애일 수 있습니다. 인터넷·시스템은 정상일 수 있으며, 기상청이 복구되면 자동 정상화됩니다. (상단 '시스템 진단'으로 확인 가능)"
                elif "timeout" in el:
                    reason = "기상청 API 응답 지연(시간 초과) — 잠시 후 자동 재시도됩니다."
                elif grid_err:
                    reason = f"기상청 API 오류: {grid_err[:150]}"
                else:
                    reason = "기상청에서 날씨 데이터가 반환되지 않았습니다."
                results.append({"site_id": site.id, "site_name": site.name, "address": site.address or "",
            ...
            # 날씨 로그 저장 (실시간 조회 성공 시에만; 캐시 폴백일 땐 중복 저장하지 않음)
            if not weather_is_cached:
                await log_repo.create(
            ...
            results.append({
                ...
                "work_date_start": site.work_date_start.isoformat() if site.work_date_start else None,
                "work_date_end": site.work_date_end.isoformat() if site.work_date_end else None,
                "work_status": site_work_status(site, datetime.now(KST).date()),
                "workers": worker_list,
                "worker_count": len(worker_list),
                "ack_count": 0,
                "is_cached": weather_is_cached,
...
async def system_diagnostics(
    ...
    """시스템 자가진단.
    담당자가 '조회 실패' 화면을 보면, 상단 [시스템 진단] 버튼을 눌러 이 API를 호출한다.
    데이터베이스 / 인터넷 / 카카오(주소→좌표) / 기상청(날씨) 를 각각 실제로 점검해서
    무엇이 정상이고 무엇이 문제인지 사람이 읽을 수 있는 형태로 돌려준다.
    → 담당자는 이 결과 화면 1장만 캡쳐하면 원인이 바로 드러난다.
    """
    import time
    import httpx
    ...
    async def http_check(name, url, ok_msg, fail_msg):
        """지정한 주소에 실제로 접속해보고 정상/실패를 판정 (HTTP 응답만 오면 '연결됨')."""
        t0 = time.perf_counter()
        try:
            async with httpx.AsyncClient(verify=False, timeout=8.0, follow_redirects=True) as c:
                r = await c.get(url)
            ms = int((time.perf_counter() - t0) * 1000)
            return {"name": name, "ok": True, "detail": f"{ok_msg} (응답 {ms}ms)", "elapsed_ms": ms}
        except Exception as e:
            ms = int((time.perf_counter() - t0) * 1000)
            et = type(e).__name__
            hint = ""
            if "Timeout" in et or "Connect" in et:
                hint = " · 해당 서버가 응답하지 않음(서버 장애 또는 방화벽 차단)"
            return {"name": name, "ok": False, "detail": f"{fail_msg}: {et}{hint} ({ms}ms)", "elapsed_ms": ms}

    # 1) 데이터베이스
    t0 = time.perf_counter()
    try:
        site_repo = WorkSiteRepository(db)
        n = len(await site_repo.get_all_outdoor_active())
        checks.append({"name": "데이터베이스", "ok": True,
                       "detail": f"정상 (활성 옥외 현장 {n}개)",
                       "elapsed_ms": int((time.perf_counter() - t0) * 1000)})
    except Exception as e:
        checks.append({"name": "데이터베이스", "ok": False,
                       "detail": f"오류: {type(e).__name__} - {str(e)[:120]}", "elapsed_ms": 0})

    # 2) 인터넷 연결(대조군) — 이게 되면 '망 전체 문제'는 아님
    checks.append(await http_check("인터넷 연결", "https://www.naver.com", "정상", "실패"))

    # 3) 카카오 지도 API (엑셀 주소 → 좌표 변환에 사용)
    checks.append(await http_check("카카오 지도(주소→좌표)", "https://dapi.kakao.com", "연결 정상", "연결 실패"))

    # 4) 기상청 날씨 API — 실제 예보 조회를 그대로 실행 (경남 창원 부근 좌표)
    t0 = time.perf_counter()
    try:
        w = await weather_provider.get_current_weather(35.2281, 128.6811)
        ms = int((time.perf_counter() - t0) * 1000)
        checks.append({"name": "기상청 날씨 API", "ok": True,
                       "detail": f"정상 (기온 {w.temperature}°C · 응답 {ms}ms)", "elapsed_ms": ms})
    except Exception as e:
        ms = int((time.perf_counter() - t0) * 1000)
        et = type(e).__name__
        if "Timeout" in et or "Connect" in et:
            detail = (f"연결 불가 — 기상청 API 서버(apihub.kma.go.kr)가 응답하지 않습니다. "
                      f"기상청 서버 일시 장애일 가능성이 높습니다. ({et} · {ms}ms)")
        else:
            detail = f"오류: {et} - {str(e)[:120]} ({ms}ms)"
        checks.append({"name": "기상청 날씨 API", "ok": False, "detail": detail, "elapsed_ms": ms})
    ...
    if all_ok:
        ...
        net_ok = any(c["name"] == "인터넷 연결" and c["ok"] for c in checks)
        kma_bad = any(c["name"] == "기상청 날씨 API" and not c["ok"] for c in checks)
        if kma_bad and net_ok and len(failed) == 1:
            summary = "인터넷은 정상이나 기상청 API만 응답하지 않습니다. → 기상청 서버 문제로, 시스템 잘못이 아닙니다. 기상청 복구 시 자동 정상화됩니다."
        else:
            summary = f"문제 발견: {', '.join(failed)}"
...
async def verify_weather(
    ...
    try:
        ...
        if T < 26.7:
            ...
            # 기상청 여름철 체감온도 산출식 (2022.6.2 개정, 습구온도 기반)
            Tw_hi = HeatIndexCalculator._wet_bulb_temperature(T, RH)
            PT = (
                -0.2442
                + 0.55399 * Tw_hi
                + 0.45535 * T
            ...
            apparent_temp = round(PT, 1)
            heat_index_detail = {
                "formula": "기상청 여름철 체감온도 산출식 (2022.6 개정)",
                "expression": "체감온도 = -0.2442 + 0.55399·Tw + 0.45535·Ta - 0.0022·Tw² + 0.00278·Tw·Ta + 3.0",
                "input_temperature": T,
                "input_humidity": RH,
                "wet_bulb_temp": round(Tw_hi, 2),
                "wet_bulb_formula": "Stull(2011) 근사식: Tw = Ta·atan[0.151977·√(RH+8.313659)] + atan(Ta+RH) - atan(RH-1.676331) + 0.00391838·RH^1.5·atan(0.023101·RH) - 4.686035",
                "terms": {
                    "상수": -0.2442,
                    "0.55399·Tw": round(0.55399 * Tw_hi, 4),
                    "0.45535·Ta": round(0.45535 * T, 4),
                    "-0.0022·Tw²": round(-0.0022 * Tw_hi * Tw_hi, 4),
                    "0.00278·Tw·Ta": round(0.00278 * Tw_hi * T, 4),
                    "보정상수": 3.0,
                },
                "raw_result": round(PT, 4),
...
async def verify_all_weather(
    ...
    for site in sites:
        ...
        # 체감온도 계산 (기상청 여름철 체감온도 산출식 - 공용 계산기 사용)
        kma_apparent = HeatIndexCalculator.calculate_heat_index(kma_temp, kma_humidity)

        # WBGT
        kma_wbgt = HeatIndexCalculator.estimate_wbgt_outdoor(
            kma_temp, kma_humidity, kma_wind
        )
