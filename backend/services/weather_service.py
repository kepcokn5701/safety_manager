...
class HeatIndexCalculator:
    """
    체감온도 및 WBGT 계산기
    - 체감온도: 기상청 여름철 체감온도 산출식 (2022.6 개정, 습구온도 기반)
    - WBGT: 간이 추정 공식 (옥외, 직사일광 기준)
    """

    @staticmethod
    def _wet_bulb_temperature(temperature: float, humidity: float) -> float:
        """
        습구온도(Tw) 추정 - Stull(2011) 근사식
    ...
    def calculate_heat_index(temperature: float, humidity: float) -> float:
        """
        체감온도 계산 - 기상청 여름철 체감온도 산출식 (2022.6.2 개정)

        체감온도 = -0.2442 + 0.55399·Tw + 0.45535·Ta
                   − 0.0022·Tw² + 0.00278·Tw·Ta + 3.0
        - Ta: 기온(°C), Tw: 습구온도(Stull 근사식)

        기상청은 여름철(5~9월) 고온 조건에서 이 식을 적용하므로,
        기온이 낮을 때(<26.7°C)는 체감온도 ≈ 기온으로 처리한다.
        (한국전력공사 경남본부 폭염 안전관리 시스템)
        """
        Ta = temperature
        RH = humidity

        # 26.7°C 미만이면 체감온도 ≈ 기온 (여름철 산출식 적용 범위 밖)
        if Ta < 26.7:
            return Ta

        Tw = HeatIndexCalculator._wet_bulb_temperature(Ta, RH)
