            ...
            <div style="display:flex;justify-content:space-between;align-items:center">
                <div style="flex:1;min-width:0">
                    <div style="display:flex;align-items:center;gap:6px">
                        ...
                        ${s.work_status ? (() => { const wd = workDateInfo(s); return `<span title="작업예정일 ${wd.range || '미인식'}" style="flex-shrink:0;font-size:9px;padding:1px 5px;background:${wd.color};color:#fff;border-radius:4px;font-weight:700">${wd.badge}</span>`; })() : ''}
                    ...
                    <div style="font-size:11px;color:var(--text-dim, #8896a6);margin-top:2px">${escHtml(s.address || '')}</div>
                    ${s.is_cached ? `<div style="font-size:10px;color:#b45309;margin-top:3px;padding:2px 6px;background:#fff7ed;border-radius:4px;display:inline-block">⚠ 기상청 일시 연결 불가 · ${escHtml(s.cached_label || '')} 저장값 표시</div>` : ''}
                    ${s.worker_count > 0 ? (() => {
...
// ── 시스템 자가진단 ──
// 상단 [🩺 시스템 진단] 버튼 → 기상청/카카오/인터넷/DB 상태를 한 번에 점검해서
// 어디가 문제인지 화면에 보기 좋게 표시한다. 담당자는 이 화면만 캡쳐하면 원인이 바로 보인다.
async function runSystemDiagnostics() {
    const btn = event?.target?.closest('button');
    const oldHtml = btn ? btn.innerHTML : '';
    if (btn) { btn.disabled = true; btn.innerHTML = '<span class="progress-spinner" style="width:12px;height:12px;border-width:2px;display:inline-block;vertical-align:middle;margin-right:4px"></span>진단 중...'; }

    // 결과를 담을 오버레이(팝업)를 즉석에서 만든다 (index.html 수정 불필요)
    ...
    overlay.innerHTML = `<div style="background:#fff;border-radius:12px;max-width:520px;width:100%;max-height:85vh;overflow:auto;box-shadow:0 10px 40px rgba(0,0,0,0.25)">
        <div style="padding:16px 20px;border-bottom:1px solid #edf2f7;display:flex;justify-content:space-between;align-items:center">
            <b style="font-size:16px">🩺 시스템 진단</b>
            <button onclick="document.getElementById('diag-overlay').remove()" style="border:none;background:none;font-size:20px;cursor:pointer;color:#8896a6">×</button>
        </div>
        <div id="diag-body" style="padding:20px;text-align:center;color:#8896a6">
            <span class="progress-spinner" style="width:20px;height:20px;border-width:2px;display:inline-block;vertical-align:middle;margin-right:8px"></span>점검 중입니다...
        </div></div>`;

    try {
        const d = await api('/api/weather/diag');
        const body = document.getElementById('diag-body');
        if (!body) return;

        const rows = (d.checks || []).map(c => {
            const icon = c.ok ? '✅' : '❌';
            const nameColor = c.ok ? '#16a34a' : '#dc2626';
            const bg = c.ok ? '#f0fdf4' : '#fef2f2';
            return `<div style="display:flex;gap:10px;padding:10px 12px;background:${bg};border-radius:8px;margin-bottom:8px;text-align:left">
                <div style="font-size:16px;flex-shrink:0">${icon}</div>
                <div style="flex:1;min-width:0">
                    <div style="font-weight:700;font-size:13px;color:${nameColor}">${escHtml(c.name)}</div>
                    <div style="font-size:12px;color:#4a5568;margin-top:2px;line-height:1.5">${escHtml(c.detail || '')}</div>
                </div></div>`;
        }).join('');

        const sumBg = d.all_ok ? '#e8f7ee' : '#fff7ed';
        const sumColor = d.all_ok ? '#15803d' : '#b45309';
        const checkedAt = d.checked_at ? new Date(d.checked_at).toLocaleString('ko-KR', {hour:'2-digit',minute:'2-digit',second:'2-digit'}) : '';

        body.style.textAlign = 'left';
        body.style.color = 'inherit';
        body.innerHTML = `
            <div style="padding:12px 14px;background:${sumBg};color:${sumColor};border-radius:8px;font-size:13px;font-weight:600;line-height:1.6;margin-bottom:14px">${escHtml(d.summary || '')}</div>
            ${rows}
            <div style="text-align:right;font-size:11px;color:#a0aec0;margin-top:8px">점검 시각 ${escHtml(checkedAt)}</div>
            <div style="text-align:center;margin-top:14px">
                <button class="btn btn-ghost btn-sm" onclick="runSystemDiagnostics()">다시 진단</button>
                <button class="btn btn-primary btn-sm" onclick="document.getElementById('diag-overlay').remove()">닫기</button>
            </div>`;
    } catch (e) {
        const body = document.getElementById('diag-body');
        if (body) {
            body.style.color = '#dc2626';
            body.innerHTML = `진단 실행 실패: ${escHtml(e.message)}<br><br>서버(앱)가 꺼져 있거나 응답하지 않을 수 있습니다.`;
        }
    } finally {
...
async function importSelectedSites() {
    ...
    // 작업예정일시(기간) 컬럼 자동 감지 → 날짜 기반 발송용
    const periodColEntry = Object.entries(excelData.mapped_columns).find(([, v]) => v === 'period');
    const periodCol = periodColEntry ? periodColEntry[0] : null;
    ...
    const sites = checked.map(i => {
        ...
        return {
            ...
            work_period: periodCol ? (row[periodCol] || '') : '',
            workers: siteWorkers,
        };
...
function workDateInfo(s) {
    const fmt = d => d ? d.slice(5).replace('-', '/') : '';
    const range = s.work_date_start
        ? ((s.work_date_end && s.work_date_end !== s.work_date_start)
            ? `${fmt(s.work_date_start)}~${fmt(s.work_date_end)}`
            : fmt(s.work_date_start))
        : '';
    switch (s.work_status) {
        case 'today':    return { badge: '오늘', color: '#16a34a', selectable: true, range };
        case 'upcoming': return { badge: '예정', color: '#2563eb', selectable: false, range };
        case 'ended':    return { badge: '종료', color: '#94a3b8', selectable: false, range };
        default:         return { badge: '날짜미인식', color: '#dc2626', selectable: false, range: '' };
    }
}
...
function renderAlertSendList() {
    ...
    // 작업일 기준: 오늘 작업 현장만 기본 선택 (요청: 작업예정일==오늘 인 건에만 발송)
    const todayCount = sendable.filter(s => workDateInfo(s).selectable).length;
    const excludedByDate = sendable.length - todayCount;

    // 컴팩트 테이블 형태
    let totalWorkerCount = 0;
    let html = '';
    if (excludedByDate > 0) {
        html += `<div style="margin:0 0 8px;padding:8px 12px;background:#fffbeb;border:1px solid #fde68a;border-radius:8px;font-size:11.5px;color:#92400e;line-height:1.5">
            &#9888;&#65039; 작업예정일이 오늘이 아니거나 <b>날짜미인식</b>인 <b>${excludedByDate}개 현장</b>은 기본 발송 대상에서 제외되었습니다.
            (작업예정일이 <b>오늘</b>인 ${todayCount}개 현장만 선택됨)
        </div>`;
    }
    html += `<table style="width:100%;border-collapse:collapse;font-size:12px">
        <thead><tr style="background:rgba(0,0,0,0.03);position:sticky;top:0;z-index:1">
            <th style="padding:6px 4px;text-align:center;width:28px"><input type="checkbox" id="alert-select-all-table" ${todayCount === sendable.length ? 'checked' : ''} onchange="toggleAlertSelectAll(this)" style="width:auto"></th>
            ...
            <th style="padding:6px 4px;text-align:center;width:78px">작업일</th>
    ...
    html += sendable.map((s, idx) => {
        ...
        const rowBg = wd.selectable ? '' : 'background:#fafafa';
        const dateBadge = `<span style="display:inline-block;padding:1px 5px;border-radius:6px;font-size:9px;font-weight:700;color:#fff;background:${wd.color}">${wd.badge}</span>${wd.range ? `<div style="font-size:9px;color:#94a3b8;margin-top:1px">${wd.range}</div>` : ''}`;

        return `<tr style="border-bottom:1px solid #f0f0f0;${rowBg}" title="${workerStr}\n${phoneStr}">
            <td style="padding:5px 4px;text-align:center"><input type="checkbox" class="alert-site-check" data-site-id="${s.site_id}" onchange="updateAlertSelectedCount()" style="width:auto" ${wd.selectable ? 'checked' : ''}></td>
            ...
            <td style="padding:5px 4px;text-align:center">${dateBadge}</td>
    ...
    html += `</tbody><tfoot><tr style="background:rgba(0,0,0,0.03);border-top:2px solid #e2e8f0">
        <td colspan="6" style="padding:6px 8px;font-size:11px;font-weight:700;text-align:right;color:#333">합계: ${sendable.length}개 현장 (오늘 작업 ${todayCount}개)</td>
...
async function sendSelectedSms() {
    ...
    try {
        ...
        if (result) {
            ...
            // 작업일 불일치·미인식으로 발송 제외된 현장 경고
            if (result.skipped_sites && result.skipped_sites.length > 0) {
                const reasonLabel = { '날짜미인식': '날짜미인식', '작업일불일치': '작업일이 오늘 아님' };
                const items = result.skipped_sites.map(s =>
                    `<li>${escHtml(s.name)} <span style="color:#b45309">(${reasonLabel[s.reason] || s.reason})</span></li>`
                ).join('');
                resultHtml += `<div style="background:#fffbeb;border:1px solid #fde68a;border-radius:8px;padding:10px 12px;margin:8px 0;font-size:12px;color:#92400e">
                    <div style="font-weight:700;margin-bottom:4px">&#9888;&#65039; 작업일 검증으로 발송 제외 (${result.skipped_sites.length}개 현장)</div>
                    <ul style="margin:4px 0 0;padding-left:18px;line-height:1.6">${items}</ul>
                    <div style="margin-top:6px;font-size:11px;color:#b45309">작업예정일이 오늘인 현장에만 발송됩니다. 날짜미인식 현장은 사전신고 엑셀의 '작업예정일시'를 확인 후 다시 업로드하세요.</div>
                </div>`;
            }
